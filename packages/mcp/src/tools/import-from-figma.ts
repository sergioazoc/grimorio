...
export function registerImportFromFigma(server: McpServer, config: McpServerConfig): void {
  server.tool(
    "import_from_figma",
    "Import a component spec from a Figma file URL. Requires FIGMA_TOKEN environment variable.",
    {
      url: z.string().describe("Figma file URL"),
      component: z.string().optional().describe("Component name to find in the file"),
    },
    async ({ url, component }) => {
      ...
      if (!figmaToken) {
        return {
          content: [
            {
              type: "text" as const,
              text: "FIGMA_TOKEN environment variable is required.",
            },
          ],
        };
      }
      ...
      if (!validation.success) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Generated spec is invalid: ${validation.error.issues.map((i) => i.message).join(", ")}`,
            },
          ],
        };
      }
      ...
      const outputPath = join(specsDir, `${componentName.toLowerCase()}.json`);
  ...
  for (const [key, prop] of Object.entries(properties)) {
    ...
    switch (prop.type) {
      case "VARIANT": {
        ...
        variants.push({ name: cleanName, values, description: `Figma variant: ${cleanName}` });
